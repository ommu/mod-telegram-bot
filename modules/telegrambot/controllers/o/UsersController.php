...
class UsersController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = TelegrambotUsers::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('TelegrambotUsers', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Telegrambot Users');
	...
	public function actionView($id) 
	{
		...
		$this->pageTitle = Yii::t('phrase', 'View Telegrambot Users');
	...
	public function actionManage() 
	{
		...
		$this->pageTitle = Yii::t('phrase', 'Telegrambot Users Manage');
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	{
		$model=new TelegrambotUsers;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['TelegrambotUsers'])) {
			$model->attributes=$_POST['TelegrambotUsers'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'TelegrambotUsers success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'TelegrambotUsers success created.'));
		...
		$this->pageTitle = Yii::t('phrase', 'Create Telegrambot Users');
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['TelegrambotUsers'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'TelegrambotUsers success updated.'));
					//$this->redirect(array('view','id'=>$model->subscribe_id));
					$this->redirect(array('manage'));
				}
			}
		...
		$this->pageTitle = Yii::t('phrase', 'Update Telegrambot Users');
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				TelegrambotUsers::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				TelegrambotUsers::model()->deleteAll($criteria);
			}
		}
	...
	{
		$model = TelegrambotUsers::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='telegrambot-users-form') {
